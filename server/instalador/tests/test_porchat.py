...
class ElCanje(Base):
    ...
    def test_de_punta_a_punta_sin_la_psk_fuera(self):
        hallado = self.lanzar()
        self.assertEqual(hallado.group(1), "443")
        ...
        self.assertEqual(datos["puerto"], 443)
    ...
    def test_con_el_443_ocupado_el_primero_libre_desde_el_8443(self):
        self.falso.tcp += [("0.0.0.0:443", "caddy"), ("[::]:8443", "otro")]
        self.assertEqual(self.lanzar().group(1), "8444")
        self.assertEqual(self.canje_json()["puerto"], 8444)

    def test_sin_puerto_libre_se_para(self):
        self.falso.tcp += [("0.0.0.0:%d" % p, "otro") for p in [443] + list(range(8443, 8454))]
    ...
    def test_con_ufw_activo_abre_el_puerto_solo_para_el_canje(self):
        ...
        self.lanzar()
        self.assertIn("ufw allow proto tcp from any to any port 443 comment hehermes-canje", self.falso.reglas_ufw)
        self.assertNotIn("ufw allow proto tcp from any to any port 443 comment hehermes-canje",
    ...
    def test_con_qr_png_deja_el_enlace_y_nada_mas(self):
        ...
        self.assertEqual(self.falso.pngs["/tmp/hehermes-canje.png"], hallado.group(0))
